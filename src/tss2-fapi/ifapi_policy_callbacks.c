 ...
 */
static void
get_nv_auth_object(IFAPI_OBJECT *nv_object,
                   ESYS_TR       nv_index,
    ...
    /* Finalize or retry the reading and check the object type */
        ...
        case IFAPI_EXT_PUB_KEY_OBJ:
            *public = object.misc.ext_pub_key.public.publicArea;
            break;
        default:
            goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "Object %s is not a key.", cleanup, path);
        }
        break;
 ...
 */
TSS2_RC
ifapi_get_object_name(const char *path, TPM2B_NAME *name, void *ctx) {
    TSS2_RC       r = TSS2_RC_SUCCESS;
    IFAPI_OBJECT  object;
    FAPI_CONTEXT *context = ctx;
    ...
    /* Finalize or retry the reading and check the object type */
        ...
        switch (object.objectType) {
        case IFAPI_KEY_OBJ:
            r = ifapi_get_name(&object.misc.key.public.publicArea, (TPM2B_NAME *)name);
            break;
        case IFAPI_EXT_PUB_KEY_OBJ:
            r = ifapi_get_name(&object.misc.ext_pub_key.public.publicArea, (TPM2B_NAME *)name);
            break;
        case IFAPI_NV_OBJ:
            r = ifapi_nv_get_name(&object.misc.nv.public.nvPublic, name);
            break;
        default:
            goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "Invalid object %s.", cleanup, path);
        }
 ...
 */
TSS2_RC
ifapi_get_nv_public(const char      *path,
    ...
    if (nv_index) {
        ...
        statecase(context->read_nv_public_state, READ_NV_PUBLIC_INIT)
            r = Esys_TR_FromTPMPublic_Async(context->esys, nv_index, ESYS_TR_NONE, ESYS_TR_NONE,
                                            ESYS_TR_NONE);
        ...
        statecase(context->read_nv_public_state, READ_NV_PUBLIC_GET_ESYS_TR)
            ...
            r = Esys_NV_ReadPublic_Async(context->esys, esys_tr, ESYS_TR_NONE, ESYS_TR_NONE,
                                         ESYS_TR_NONE);
            goto_if_error(r, "Error: nv read public async", cleanup);
            fallthrough;

        statecase(context->read_nv_public_state, READ_NV_PUBLIC_GET_PUBLIC)
            r = Esys_NV_ReadPublic_Finish(context->esys, &nv_public_esys, NULL);
 ...
 */
TSS2_RC
ifapi_read_pcr(TSS2_POLICY_PCR_SELECTION *pcr_selection,
     ...
     * the raw selector over to the input selector */
        if (pcr_selection->type == TSS2_POLICY_PCR_SELECTOR_PCR_SELECT) {
            ...
            for (i = 0; i < profile_selection->count; i++) {
                for (pcr = 0; pcr < TPM2_MAX_PCRS; pcr++) {
                    ...
                    if ((flag & profile_selection->pcrSelections[i].pcrSelect[byte_idx])
                        ...
                        in_pcr_selection.pcrSelections[0].hash
                            = profile_selection->pcrSelections[i].hash;
                    }
                }
            }
            ...
            for (i = 0; i < pcr_selection->selections.pcr_select.sizeofSelect; i++)
                in_pcr_selection.pcrSelections[0].pcrSelect[i]
                    = pcr_selection->selections.pcr_select.pcrSelect[i];
        } else {
    ...
    statecase(context->io_state, IO_ACTIVE)
    /* Finalize or retry the reading and check the object type */
        r = Esys_PCR_Read_Finish(context->esys, &update_counter, &out_selection, &pcr_digests);
 ...
 */
TSS2_RC
ifapi_policyeval_cbauth(TPM2B_NAME *name,
    ...
    statecase(cb_ctx->cb_state, POL_CB_EXECUTE_INIT);
        ...
        r = ifapi_keystore_search_obj(&fapi_ctx->keystore, &fapi_ctx->io, name,
                                      &cb_ctx->object_path);
        FAPI_SYNC(r, "Search Object", cleanup);

        r = ifapi_keystore_load_async(&fapi_ctx->keystore, &fapi_ctx->io, cb_ctx->object_path);
    ...
    statecase(cb_ctx->cb_state, POL_CB_READ_OBJECT);
        ...
        r = ifapi_keystore_load_finish(&fapi_ctx->keystore, &fapi_ctx->io, &cb_ctx->object);
        return_try_again(r);
        return_if_error(r, "read_finish failed");
        ...
        if (cb_ctx->object.objectType == IFAPI_NV_OBJ) {
            ...
            get_nv_auth_object(&cb_ctx->object, cb_ctx->object.public.handle, &cb_ctx->auth_object,
                               &cb_ctx->auth_index);
...
cleanup:
    ifapi_cleanup_ifapi_object(&cb_ctx->object);
    if (current_policy->policySessionSav && current_policy->policySessionSav != ESYS_TR_NONE)
        fapi_ctx->policy.session = current_policy->policySessionSav;
    return r;
}
 ...
 */
TSS2_RC
ifapi_policyeval_cbload_key(TPM2B_NAME *name, ESYS_TR *object_handle, void *userdata) {
    TSS2_RC                   r;
    FAPI_CONTEXT             *fapi_ctx = userdata;
    IFAPI_POLICY_EXEC_CTX    *current_policy;
    IFAPI_POLICY_EXEC_CB_CTX *cb_ctx;
    ...
    statecase(cb_ctx->cb_state, POL_CB_READ_OBJECT);
        ...
        if (cb_ctx->object.objectType != IFAPI_KEY_OBJ) {
            goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "Key object expected", cleanup);
        }
        cb_ctx->key_handle = cb_ctx->object.public.handle;
        if (cb_ctx->key_handle != ESYS_TR_NONE) {
            break;
 ...
 */
TSS2_RC
ifapi_branch_selection(TSS2_OBJECT *auth_object,
    ...
    return_if_null(fapi_ctx->callbacks.branch, "No branch selection callback",
                   TSS2_FAPI_RC_AUTHORIZATION_UNKNOWN);

    object_path = ifapi_get_object_path(TSS2_OBJECT_TO_IFAPI_OBJECT(auth_object));

    r = fapi_ctx->callbacks.branch(object_path, "PolicyOR", &branch_names[0], branch_count,
                                   branch_idx, fapi_ctx->callbacks.branchData);
 ...
 */
TSS2_RC
ifapi_policy_action(const char *action, void *userdata) {
    TSS2_RC       r;
    FAPI_CONTEXT *fapi_ctx = userdata;
    IFAPI_OBJECT *auth_object;
    const char   *object_path;
    ...
    if (!fapi_ctx->callbacks.action) {
        return_error(TSS2_FAPI_RC_AUTHORIZATION_UNKNOWN, "No action callback");
    }
    ...
    r = fapi_ctx->callbacks.action(object_path, action, fapi_ctx->callbacks.actionData);
 ...
 */
TSS2_RC
ifapi_sign_buffer(char           *key_pem,
    ...
    if (!fapi_ctx->callbacks.sign) {
        return_error2(TSS2_FAPI_RC_AUTHORIZATION_UNKNOWN, "No signature callback.");
    }
    r = fapi_ctx->callbacks.sign(object_path, "PolicySigned", key_pem,
                                 public_key_hint ? public_key_hint : "", key_pem_hash_alg, buffer,
                                 buffer_size, signature, signature_size,
 ...
 */
static TSS2_RC
equal_policy_authorization(TPMS_POLICY *policy,
                           void        *publicVoid,
                           void        *policyRefVoid,
                           bool        *equal) {
    ...
    if (authorizations) {
        for (i = 0; i < authorizations->count; i++) {
            if (strcmp(authorizations->authorizations[i].type, "pem") == 0) {
                ...
                r = ifapi_get_tpm2b_public_from_pem(authorizations->authorizations[i].keyPEM,
                                                    &pem_public);
 ...
 */
static TSS2_RC
compare_policy_digest(TPMS_POLICY *policy, void *authPolicyVoid, void *nameAlgVoid, bool *equal) {
    ...
    if (digest_values) {
        for (i = 0; i < digest_values->count; i++) {
            if (digest_values->digests[i].hashAlg == hash_alg) {
                if (memcmp(&digest_values->digests[i].digest, &authPolicy->buffer[0],
                           authPolicy->size)
                    != 0)
                    continue;
                *equal = true;
 ...
 */
static TSS2_RC
search_policy(FAPI_CONTEXT         *context,
    ...
    case FSEARCH_INIT:
        ...
        r = ifapi_keystore_list_all(&context->keystore, IFAPI_POLICY_DIR,
                                    &context->fsearch.pathlist, &context->fsearch.numPaths);
    ...
    case FSEARCH_READ:
        ...
        if (found) {
            LOG_DEBUG("compare true  %s", context->fsearch.pathlist[context->fsearch.path_idx]);
        } else {
            LOG_DEBUG("compare false  %s", context->fsearch.pathlist[context->fsearch.path_idx]);
        }
 ...
 */
static TSS2_RC
get_policy_digest(TPMS_POLICY *policy, TPMI_ALG_HASH hashAlg, TPM2B_DIGEST *digest) {
    size_t i;

    if (!(digest->size = ifapi_hash_get_digest_size(hashAlg))) {
        return_error2(TSS2_FAPI_RC_BAD_VALUE, "Unsupported hash algorithm (%" PRIu16 ")", hashAlg);
    }

    for (i = 0; i < policy->policyDigests.count; i++) {
        if (policy->policyDigests.digests[i].hashAlg == hashAlg) {
            memcpy(&digest->buffer[0], &policy->policyDigests.digests[i].digest, digest->size);
            return TSS2_RC_SUCCESS;
        }
    }
 ...
 */
static TSS2_RC
get_policy_signature(TPMS_POLICY *policy, TPMT_PUBLIC *public, TPMT_SIGNATURE *signature) {
    TSS2_RC                    r;
    size_t                     i;
    TPM2B_PUBLIC               pem_public;
    TPML_POLICYAUTHORIZATIONS *authorizations = policy->policyAuthorizations;
        ...
        if (strcmp(authorizations->authorizations[i].type, "pem") == 0) {
            ...
            if (ifapi_TPMT_PUBLIC_cmp(public, &pem_public.publicArea)) {
                r = ifapi_der_sig_to_tpm(&pem_public.publicArea,
                                         ...
                                         authorizations->authorizations[i].hashAlg, signature);
 ...
 */
TSS2_RC
ifapi_exec_auth_policy(TPMT_PUBLIC    *key_public,
    ...
    statecase(cb_ctx->cb_state, POL_CB_SEARCH_POLICY)
        r = search_policy(fapi_ctx, equal_policy_authorization, true, key_public, policyRef,
        ...
        if (current_policy->policy_list->next) {
            ...
            if (!fapi_ctx->callbacks.branch) {
                return_error(TSS2_FAPI_RC_AUTHORIZATION_UNKNOWN, "No branch selection callback");
            }
            n = 1;

            /* Count policies */
            for (branch = current_policy->policy_list; branch->next; branch = branch->next)
                n += 1;
            ...
            /* Determine path of object to be authenticated. */
            auth_object = fapi_ctx->policy.util_current_policy->pol_exec_ctx->auth_object;
            goto_if_null2(auth_object, "No object passed.", r, TSS2_FAPI_RC_BAD_REFERENCE, cleanup);

            object_path = ifapi_get_object_path(auth_object);

            /* Policy selection */
            r = fapi_ctx->callbacks.branch(object_path, "PolicyAuthorize", &names[0], n,
                                           &branch_idx, fapi_ctx->callbacks.branchData);
            goto_if_error(r, "policyBranchSelectionCallback", cleanup);

            if (branch_idx >= n) {
                goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "Invalid branch number.", cleanup);
            }
        ...
        if (!policy_set) {
            goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "Policy could not be set.", cleanup);
        }
    ...
    statecase(cb_ctx->cb_state, POL_CB_EXECUTE_SUB_POLICY)
        ...
        r = get_policy_signature(cb_ctx->policy, key_public, signature);
 ...
 */
TSS2_RC
ifapi_exec_auth_nv_policy(TPMS_NV_PUBLIC *nv_public, TPMI_ALG_HASH hash_alg, void *userdata) {
    TSS2_RC                   r;
    ...
    if (!(digest_size = ifapi_hash_get_digest_size(hash_alg))) {
        return_error2(TSS2_FAPI_RC_BAD_VALUE, "Unsupported hash algorithm (%" PRIu16 ")", hash_alg);
    }
     ...
     */
        ...
        r = ifapi_keystore_search_nv_obj(&fapi_ctx->keystore, &fapi_ctx->io, &tmp, &nv_path);
        FAPI_SYNC(r, "Search Object", cleanup);
    ...
    /* Get object from file */
        ...
        get_nv_auth_object(&cb_ctx->object, current_policy->nv_index,
                           &current_policy->auth_objectNV, &current_policy->auth_handle);
        fallthrough;
    ...
    /* Authorize the NV object with the corresponding auth object. */
        ...
        r = Esys_NV_Read_Async(esys_ctx, current_policy->auth_handle, current_policy->nv_index,
    ...
    /* Finalize the reading. */
        ...
        offset = 0;
        r = Tss2_MU_TPMT_HA_Unmarshal(&aux_data->buffer[0], aux_data->size, &offset, &nv_policy);
        Esys_Free(aux_data);
        goto_if_error(r, "Unmarshal policy", cleanup);
    ...
    /* Search policy appropriate in object store */
        r = search_policy(fapi_ctx, compare_policy_digest, false, &cb_ctx->policy_digest, &hash_alg,
                          &current_policy->policy_list);
        FAPI_SYNC(r, "Search policy", cleanup);
 ...
 */
TSS2_RC
ifapi_get_duplicate_name(TPM2B_NAME *name, void *userdata) {
    FAPI_CONTEXT *fapi_ctx = userdata;
